...
void CFixedHtmlView::GetLocationURL(CString &strLocURL)
{
	if (m_b01InUrl)
	{
		strLocURL = ((CGreenBrowserView *)this)->GetUrl();
		return;
	}
	BSTR bstr = NULL;
	try
									 ...
									 VARIANT *PostData, VARIANT *Headers, BOOL *Cancel)
{
	...
	try
	{
		...
		if (m_strUrl.Find(0x01) >= 0 && m_strUrl.Find('@') >= 0)
		{
			...
			m_b01InUrl = TRUE;
			((CGreenBrowserView *)this)->SetUrl(m_strUrl);
		}
		else
			m_b01InUrl = FALSE;
		...
		if (pmf->m_bUseTargetFrameName)
		{
			...
			if (strTargetFrameName.GetLength() && strTargetFrameName.Find("No__Name") < 0) //{"_No__Name:4" VT_BSTR}
			{
				...
				if (VALID_TCF(tcfTarget) &&
					...
					((CGreenBrowserView *)this)->GetUrl().IsEmpty())
				{
					*Cancel = TRUE;
					m_bToClose = TRUE;
					tcf->PostMessage(WM_CLOSE);
...
BOOL CFixedHtmlView::PreTranslateMessage(MSG *pMsg)
{
	...
	try
	{
		...
		{
			// If a link is pressed for a long time and then released, the new link will be filtered.
			QueryPerformanceCounter(&m_tLastClick);
			m_bLClick = TRUE;
		...
		// Don't use the up message here, because the cursor will change if it's down.
...
int CFixedHtmlView::SetHistory(LPCSTR strCurUrl)
{
	if (!m_bIniTravelLog)
	{
		...
		if (pDisp != NULL)
		{
			...
			{
				hr = pISP->QueryService(SID_STravelLogCursor, IID_MyITravelLogStg, (void **)&m_pITravelLog);
				if (FAILED(hr))
					m_pITravelLog = NULL;
				pISP->Release();
			}
