...
void CChildFrame::OnClose() 
{
	...
	try
	{
		if(pmf!=NULL)
		{
			...
			{
				if (this->m_pView->GetUrl() == "about:blank")
					return;
			}
			int nTabID;
			...
			if(nTabID>=0 && nTabID<cn)
			{
				...
				if(m_pView!=NULL)
				{
					...
					if(m_pView->GetUrl()!="about:blank")
					{
						pmf->m_strUndoURL[pmf->m_nUndoEnd] = m_pView->GetUrl();
...
void CChildFrame::OnMDIActivate(BOOL bActivate, CWnd* pActivateWnd, CWnd* pDeactivateWnd) 
{
	...
	{
		if (m_pView->GetUrl() && !m_pView->GetUrl().IsEmpty())
			pmf->SetAddress(m_pView->GetUrl());
